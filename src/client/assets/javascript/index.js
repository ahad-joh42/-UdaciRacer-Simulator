...
function setupClickHandlers() {
	document.addEventListener('click', function(event) {
		const { target } = event
		const contarget = event.target.parentElement
		...
		if (contarget.matches('.card.track')) {
			handleSelectTrack(contarget)
		}
	
			
		if (contarget.matches('.card.podracer')) {
			handleSelectPodRacer(contarget)
		} 
...
const racesName ={
	"Racer 1":"Erik Aaby",
	"Racer 2":"Bert de Jong",
	"Racer 3":"Francisco Alcuaz",
	"Racer 4":"Yoann Bonato",
	"Racer 5": "Yazeed Al-Rajhi",
}
...
function raceProgress(positions) {
	...
	const results = positions.map(p => {
		return `
			<tr>
				<td>
					<h3>${count++} - ${racesName[p.driver_name]}</h3>
				</td>
			</tr>
		`
	})
...
async function getRacers() {
    ...
    .then(data => data.json())
    .catch(error => console.log(`Could not getRacers :`,error))
}
...
async function getRace(id) {
    ...
    .then(res => res.json())
    .catch(err => console.log(`Could not get cers :`,error))
}
...
async function accelerate(id) {
    ...
    return await fetch(`${SERVER}/api/races/${convertId}/accelerate`,{
        method :'POST',
        ...defaultFetchOpts(),
        body: ''
    })
